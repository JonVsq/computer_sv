...
switch ($opcion) {
    ...
    case 'modificar': {
            ...
            $catClienteJ = new ClienteJuridico();
            $respuesta = array();
            $existe = $catClienteJ->camposUnicosModificarC(array(
                "nombre" => $txt_nombre, "telefono" => $txt_telefono,
            ), "codigo", $txt_codigo);
            if ($existe['existe'] == 1) {
                $respuesta[] = array(
                ...
                if (is_numeric($idCategoria)) {
                    ...
                    if ($existeRaiz) {
                        ...
                        $subirEstado = move_uploaded_file($estadoResultado["tmp_name"], $rutaCliente . "/" . $nombreEstado);
                        if ($subirBalance && $subirEstado) {
                            if ($catClienteJ->modificarCliente(array(
                                $txt_codigo, $idCategoria, $txt_nombre,
                                $txt_telefono, $txt_direccion, date("Y-m-d")
                            ))) {
                                if ($catClienteJ->modificarDatosC(array(
                                    $txt_activoCorriente, $txt_pasivoCorriente,
                                    $txt_inventario, $nombreBalance, $nombreEstado, $txt_codigo
                                ))) {
                                    $respuesta[] = array(
                                        "estado" => 1,
                                        "encabezado" => "EXITO.",
